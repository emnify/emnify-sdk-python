...
class EMnifyTest(TestCase):
    ...
    def test_create_device(self):
        ...
        for _ in range(0, 2):
            ...
            tariff_profile = emnify.devices.tariff_profile_model(
                id=devices[0].tariff_profile.id
            )
            service_profile = emnify.devices.service_profile_model(
    ...
    def test_activate_disactivate_device(self):
        ...
        active_device_with_sim = [
            i
            for i in devices
            if i.sim and i.status.id == emnify_const.DeviceStatuses.ENABLED_ID.value
        ][0]
        ...
        for device_id in [-1, None]:
            with self.subTest(device_id=device_id):
                ...
                with pytest.raises(emnify_errors.ValidationErrorException):
                    emnify.devices.update_device(
                        device_id=device_id, device=device_model_for_update
                    )

    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_update_device_error_handling_service_profile.yaml"
    )
    def test_update_device_error_handling_service_profile_id(self):
        ...
        service_profile_for_update = emnify.devices.service_profile_model(
            id=non_existing_service_profile_id
        )
        device_model_for_update = emnify.devices.device_update_model(
            service_profile=service_profile_for_update
        )
    ...
    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_list_device_blacklist.yaml"
    )
    def test_list_device_blacklist(self):
        emnify = emnify_client(app_token=self.token)
        device_id = 11380018
        operators = [
            i for i in emnify.devices.get_device_operator_blacklist(device_id=device_id)
        ]
        self.assertGreater(len(operators), 0)
        self.assertEqual(operators[0].country.name, "Albania")

    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_delete_blacklist_operator.yaml"
    )
    def test_delete_blacklist_operator(self):
        ...
        emnify.devices.delete_device_blacklist_operator(
            device_id=device_id, operator_id=previous_operators_list[0].id
        )
        updated_operators_list = [
            i for i in emnify.devices.get_device_operator_blacklist(device_id=device_id)
        ]
    ...
    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_add_blacklist_operator.yaml"
    )
    ...
    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_add_blacklist_operator_error.yaml"
    )
    ...
    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_reset_connectivity_network.yaml"
    )
    ...
    @vcr_instance.use_cassette(
        "tests/fixtures/cassettes/test_get_device_connectivity.yaml"
    )
    def test_get_device_connectivity_data(self):
        emnify = emnify_client(app_token=self.token)
        device_id = 12132821
        connectivity_data = emnify.devices.get_device_connectivity_status(
            device_id=device_id
        )
